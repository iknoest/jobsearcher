...
- [ ] R4 — Sheets + Email redesign (Apply/Review/Skip, trimmed schema)
  - [ ] R4-02 trust-first email digest (sub-score bars, "why Apply / watch-outs / what's holding this back", funnel with 3 bands, feedback pending + summary, Keep/Reject/Note card actions, deterministic "why" generator — NO LLM) — NEXT
  - [ ] R4-01 Sheets schema + drop-stage audit trail (Jobs ~22 cols, Raw with drop_stage/drop_reason for every row, Feedback append-only, `scripts/audit_url.py` for manual lookup)
