...
_PENDING_SKIP_FILE = os.path.join(os.path.dirname(__file__), "..", "output", "pending_skip.json")


def _save_pending_skip(user_id: int, pending: dict):
    """Persist pending_skip to disk so it survives bot restarts."""
    try:
        data = {
            "user_id": user_id,
            "pending": {
                **pending,
                "selected": list(pending.get("selected", set())),  # set → list for JSON
            },
        }
        with open(_PENDING_SKIP_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f)
    except Exception:
        pass


def _load_pending_skip(user_id: int):
    """Restore pending_skip from disk if it belongs to this user."""
    try:
        with open(_PENDING_SKIP_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        if data.get("user_id") == user_id:
            pending = data["pending"]
            pending["selected"] = set(pending.get("selected", []))  # list → set
            return pending
    except Exception:
        pass
    return None
...
def _clear_pending_skip():
    try:
        os.remove(_PENDING_SKIP_FILE)
    except Exception:
        pass


def _lookup_job_from_cache(job_id: str) -> dict:
    """Fallback: find job metadata in enriched_jobs.pkl when Sheet lookup fails.

    Handles 'in_<jk>' (Indeed) and plain numeric (LinkedIn) IDs.
    Returns a partial rec dict with Title, Company, Score, Decision if found.
    """
    cache_path = os.path.join(os.path.dirname(__file__), "..", "output", "enriched_jobs.pkl")
    try:
    ...
    needle = job_id[3:] if job_id.startswith(("in_", "hx_")) else job_id

    def _url_matches(url):
        return needle.lower() in str(url).lower()

    mask = df.get("job_url", pd.Series(dtype=str)).apply(_url_matches)
    if not mask.any():
        return {}
    row = df[mask].iloc[0]
    return {
...
def _find_job_row(ws, url_or_id: str):
    ...
    raw_id = url_or_id.strip()
    # Decode prefixed IDs generated by notifier._job_id_from_url:
    #   'in_XXXX' -> Indeed jk param value (search as substring in URL)
    #   'hx_XXXX' -> MD5 hash (search as substring in URL)
    #   pure digits -> LinkedIn numeric ID
    if raw_id.startswith("in_") or raw_id.startswith("hx_"):
        needle = raw_id[3:].lower()  # strip 2-char prefix + underscore
    else:
        needle = raw_id.lower()
    ...
    for row_num, row_values in enumerate(all_values[1:], start=2):
        ...
        # Strategy 1: known URL column contains needle
        if url_col_idx is not None and url_col_idx < len(row_values):
            cell = row_values[url_col_idx].lower()
            if needle in cell:
                return row_num, rec

        # Strategy 2: any URL-like cell contains needle
        for cell in row_values:
            cell_l = cell.lower()
            if ("http" in cell_l or "linkedin" in cell_l) and needle in cell_l:
                return row_num, rec

        # Strategy 3: digit-run regex match against any URL-like cell (LinkedIn IDs)
        if needle.isdigit():
            for cell in row_values:
...
_BAD_URLS = {"unknown", "nan", "none", ""}


def _canonical_url(rec: dict, job_id: str) -> str:
    """Return a usable URL for a job.

    Prefers the Sheet's Job URL column value.  Falls back to constructing a
    LinkedIn job URL from the numeric job_id when the Sheet value is absent,
    'Unknown', 'nan', etc.  This prevents every verdict being keyed on the
    literal string "Unknown" which causes supersession collisions.
    """
    stored = str(rec.get("Job URL", "")).strip()
    if stored.lower() not in _BAD_URLS and stored.startswith("http"):
        return stored
...
async def _handle_good_deeplink(update: Update, job_id: str):
    ...
    try:
        ...
        if row_index is None:
            rec = _lookup_job_from_cache(job_id)
...
async def _handle_skip_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE, job_id: str):
    ...
    try:
        ...
        row_index, rec = _find_job_row(ws, job_id)
        if row_index is None:
            # Job not in sheet (e.g. --resend run). Fall back to enriched cache for display.
            rec = _lookup_job_from_cache(job_id)
            ws = None
        ...
        # Store state in user_data AND on disk (survives bot restart)
        pending = {
            "job_id": job_id,
        ...
        context.user_data["pending_skip"] = pending
        context.user_data["awaiting_skip_reason"] = False
        _save_pending_skip(update.effective_user.id, pending)
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get("pending_skip"):
        ...
        if restored:
            context.user_data["pending_skip"] = restored
            context.user_data["awaiting_skip_reason"] = True
...
async def _write_feedback(update: Update, url_or_id: str, verdict: str, reason: str):
    ...
    try:
        ...
        record_verdict(
            job_url=_canonical_url(rec, url_or_id),
...
async def cmd_goodmatches(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for v in reversed(good):  # most recent first
        ...
        company = _h(v.get("company", ""))
        score = v.get("system_score", "?")
        url = v.get("job_url", "")
        company_str = f"  {company}" if company else ""
        link = f'<a href="{url}">View job →</a>' if url.startswith("http") else url
        lines.append(f"• <b>{title}</b>{company_str}  (Score {score})  {ts}\n  {link}")
...
def _clear_stale_connection():
    """Call getUpdates once with a short timeout to evict any stale Telegram long-poll
    session before starting the real polling loop. Prevents the 'Conflict: terminated
    by other getUpdates request' error when the bot is restarted quickly."""
    import urllib.request
    import urllib.error
    try:
        url = f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates?timeout=1&offset=-1"
        urllib.request.urlopen(url, timeout=10)
        print("  Telegram session claimed.")
    except Exception:
        pass  # best-effort; polling will handle any remaining conflict with retries
